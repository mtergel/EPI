...
BinaryTreeNode<int>* Lca(const unique_ptr<BinaryTreeNode<int>>& node0,
  ...
  std::unordered_set<const BinaryTreeNode<int>*> nodes_on_path_to_root;
  while (iter0 || iter1) {
    if (iter0) {
      // false if insertion did not happen, aka element already in set
  ...
  throw std::invalid_argument("node0, node1 are not in the same tree");
}
